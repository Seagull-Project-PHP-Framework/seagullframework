<?php
$defaultWords['Enable logs'] = 'Logs einschalten';
$defaultWords['Log name'] = 'Name des Logs';
$defaultWords['Priority'] = 'Rangfolge';
$defaultWords['Identifier'] = 'Kennung';
$defaultWords['Target username'] = 'Ziel Benutzername';
$defaultWords['Target password'] = 'Ziel Passwort';
$defaultWords['Email options'] = 'Email Optionen';
$defaultWords['Popup window options'] = 'Popup-Fenster Optionen';
$defaultWords['Default popup window height'] = 'H�he des Popups';
$defaultWords['Default popup window width'] = 'Breite des Popups';
$defaultWords['Censorship'] = 'Zensur';
$defaultWords['Replace word with'] = 'Wort ersetzen mit';
$defaultWords['Disallowed word'] = 'Verbotene W�rter';
$defaultWords['P3P privacy policy'] = 'P3P Datenschutzrichtlinie';
$defaultWords['Policies'] = 'Richtlinie';
$defaultWords['Policy location'] = 'Ort der Richtlinie';
$defaultWords['Compact policy'] = 'Kurze Richtlinie';
$defaultWords['Use the database session handler if youre running a load-balanced environment'] = 'Benutzen Sie eine Datenbank wenn sie eine symmetrisch belastete Umgebung verwenden';
$defaultWords['This is the module that will be loaded if none are specified, ie, when you call index.php (FC only)'] = 'Dieses Modul wird geladen, wenn kein anderes angegeben ist, z.B. wenn man nur index.php aufruft (nur mit Frontcontroller)';
$defaultWords['This is the manager class that will be loaded if none are specified (FC only)'] = 'Diese Managerklasse wird geladen wenn keine andere angegeben ist (nur mit Frontcontroller)';
$defaultWords['Disable navigation altogether with this switch'] = 'Hier k�nnen Sie die Navigation ganz ausschalten';
$defaultWords['Use this option to choose from various menu types - currently only 1 provided'] = 'Diese Option erm�glicht es Ihnen, vom mehreren Menutypen auszuw�hlen. Im Moment gibt es nur einen Typ...';
$defaultWords['Defines the appearance of the navigation menu. Preview and make additional changes in the navigation module manager'] = 'Welchen Menu-Treiber wollen Sie? Vorschau und weitere Einstellm�glichkeiten finden Sie im Modul Navigation';
$defaultWords['Debugging easier when this is disabled'] = 'Debuggen ist leichter, wenn es ausgeschaltet ist';
$defaultWords['Customise the way errors are handled'] = 'Benutzerdefinierte f�r den Umgang mit Fehlern';
$defaultWords['If your IP appears in the TRUSTED_IPS array, you will be able to view system errors on screen even in production mode (see below)'] = 'Wenn Ihre IP im TRUSTED_IPS-Array vorhanden ist, k�nnen Sie Systemfehler sogar im Produktionsstatus (siehe unten) sehen';
$defaultWords['Setting this to true will disable all screen-based error messages'] = 'Wenn ja werden alle Bildschirmmeldungen ausgeschaltet';
$defaultWords['Requires to Xdebug extension to be installed'] = 'Ben�tigt PHP\'s Xdebug-Erweiterung';
$defaultWords['Errors must be >= this level before they are emailed to the site admin'] = 'Fehler m�ssen gr��er/gleich diesem Fehlerlevel sein, damit sie an den Administrator geschickt werden';
$defaultWords['It is recommended to disable this while developing'] = 'Wir empfehlen dies w�hrend der Entwicklungsphase auszuschalten';
$defaultWords['Default is 24 hours'] = 'Voreinstellung ist 24 Stunden';
$defaultWords['Make sure you load the relevant schema'] = 'Bitte stellen Sie sicher, dass sie das richige Schema geladen haben';
$defaultWords['It is recommended to disable logging if you are running < PHP 4.3.x'] = 'Empfohlen bei ausgeschaltetem Logging und PHP < 4.3.0';
$defaultWords['If sql is used, use log_table as the log table name below'] = 'Wenn sql benutzt wird tragen Sie bitte den Tabellennamen als Name des Logs ein';
$defaultWords['Use an absolute path or one relative to the Seagull root dir'] = 'Absoluter Pfad oder relativ zu Seagulls Installationsverzeichnis';
$defaultWords['Error messages get sent here'] = 'Fehlermeldungen werden an diese Adresse versandt';
$defaultWords['Contact us enquiries get sent here'] = 'Contact us Meldungen werden an diese Adresse versandt';
$defaultWords['This will be your session identifier'] = 'Kennung der Session';
$defaultWords['Disallowed words'] = 'Verbotene W�rter';
$defaultWords['Enable Safe deleting'] = 'Sicheres L�schen einschalten';
$defaultWords['Default params'] = 'Voreingestellter Parameter';
$defaultWords['Use these params to specify, eg, a static article for your homepage'] = 'z.B. f�r einen Statischen Artikel als Homepage';
$defaultWords['file'] = 'Datei';
$defaultWords['database'] = 'Datenbank';
$defaultWords['Default manager'] = 'Voreingestellter Manager';
$defaultWords['Navigation Options'] = 'Optionen f�r die Navigation';
$defaultWords['Enable Navigation'] = 'Navigation einschalten';
$defaultWords['Navigation type (driver)'] = 'Navigationstyp (Treiber)';
$defaultWords['Enable authentication'] = 'Authentifizierung einschalten';
$defaultWords['Enable custom error handler'] = 'Benutzerdefinierte Fehlersteuerung';
$defaultWords['Enable debug session'] = 'Debug-Session einschalten';
$defaultWords['Show backtrace'] = 'Backtrace anzeigen';
$defaultWords['Enable Profiling'] = 'Profiling einschalten';
$defaultWords['Email admin threshold'] = 'Schwellwert f�r Emails an den Administrator';
$defaultWords['Database Options'] = 'Optionen der Datenbank';
$defaultWords['Database Table Mappings'] = 'Datenbanktabellen Zuweisungen';
$defaultWords['Logging options'] = 'Optionen zum Logging';
$defaultWords['Session max lifetime (secs)'] = 'Max. Dauer der Session (sekunden)';
$defaultWords['Enable IP banning'] = 'IP-Verbannung erlauben';
$defaultWords['Enable Tidy html cleaning'] = 'Tidy HTML-Bereinigung erm�glichen';
$defaultWords['Session handler'] = 'Sessionsteuerung';
$defaultWords['Enable left/right columns'] = 'Linke und Rechte Spalten einschalten';
$defaultWords['Default article view type'] = 'Standardtyp der Artikel';
$defaultWords['Front controller script name'] = 'Name des Front Controller Scripts';
$defaultWords['Default module'] = 'Voreingestelltes Modul';
$defaultWords['Lists'] = 'Listen';
$defaultWords['General Site Options'] = 'Allgemeine Seitenoptionen';
$defaultWords['Base URL'] = 'Basis URL';
$defaultWords['If path to image is specified, image will be shown; if left blank, Site name from above will appear as text'] = 'Wird ein Pfad zum Bild angegeben, wird dieses angezeigt, ansonsten der Name der Seite als Text';
$defaultWords['Handy if you dont have access to Apache configuration'] = 'Praktisch, wenn man keinen direkten Zugang zur Konfiguration des Webservers hat';
$defaultWords['This way no content items are really deleting from DB, just marked as deleted'] = 'So werden die Datenbankeintr�ge nicht wirklich gel�scht sondern nur als gel�scht markiert.';
$defaultWords['Requires the tidy extension to be installed'] = 'Ben�tigt die Tidy-Erweiterung';
$defaultWords['You can turn the left/right columns off globally'] = 'Hier k�nnen Sie die linken und Rechten Spalten global ein- und ausschalten';
$defaultWords['This options allows you to change the default type of article displayed. Default Article View Type: Html Articles (2)'] = 'Diese Option erm�glicht es, den Standardtyp der Artikel anzugeben. Voreinstellung ist HTML Artikel (2)';
$defaultWords['E-mail'] = 'E-mail';
$defaultWords['Subscribe'] = 'Eintragen';
$defaultWords['Unsubscribe'] = 'Austragen';
$defaultWords['Send'] = 'Abschicken';
$defaultWords['Email'] = 'Email';
$defaultWords['Please use the following form to edit your config file'] = 'Bitte benutzen Sie dieses Formular, um die Konfigurationsdatei (conf.ini) zu bearbeiten';
$defaultWords['Site name'] = 'Name der Seite';
$defaultWords['Show logo'] = 'Logo anzeigen';
$defaultWords['Gzip compression'] = 'Gzip Kompression';
$defaultWords['Output buffering'] = 'Puffern der Ausgabe';
$defaultWords['Navigation menu stylesheet'] = 'Stylesheet des Navigationsmenus';
$defaultWords['Debugging Options'] = 'Optionen zum Debuggen';
$defaultWords['Production website'] = 'Produktionsstatus';
$defaultWords['Caching Options'] = 'Optionen zum Cachen';
$defaultWords['Enable caching'] = 'Cache einschalten';
$defaultWords['Cache lifetime (secs)'] = 'Dauer der Speicherung (in sekunden)';
$defaultWords['Type'] = 'Typ';
$defaultWords['Host'] = 'Host';
$defaultWords['Port'] = 'Port';
$defaultWords['Protocol'] = 'Protokoll';
$defaultWords['DB username'] = 'DB Benutzername';
$defaultWords['DB password'] = 'DB Passwort';
$defaultWords['DB name'] = 'DB Name';
$defaultWords['Admin contact'] = 'Administrations-Kontakt';
$defaultWords['Support contact'] = 'Support-Kontakt';
$defaultWords['Info contact'] = 'Informations-Kontakt';
$defaultWords['Cookie options'] = 'Cookie Optionen';
$defaultWords['Path'] = 'Pfad';
$defaultWords['Domain'] = 'Domain';
$defaultWords['Secure'] = 'Sicherheit';
$defaultWords['Mode'] = 'Modus';
$defaultWords['Zero means until the browser is closed'] = 'Null = bis der Browser geschlossen wird';
$defaultWords['seconds'] = 'Sekunden';
$defaultWords['queries'] = 'Datenbankabfragen';
$defaultWords['Seagull framework homepage'] = 'Homepage des Seagull Frameworks';
$defaultWords['Seagull Framework'] = 'Seagull Framework';
$defaultWords['Password'] = 'Passwort';
$defaultWords['Login'] = 'Einloggen';
$defaultWords['Forgot Password'] = 'Passwort vergessen';
$defaultWords['Not Registered'] = 'Nicht registriert';
$defaultWords['you do not have perms'] = 'Sie haben keine Berechtigung(en)';
$defaultWords['you are not allowed to access this data'] = 'Sie d�rfen diese Daten nicht bearbeiten';
$defaultWords['this element has been deleted'] = 'Dieses Element wurde gel�scht';
$defaultWords['Return to browse'] = 'Zur�ck zur �bersicht';
$defaultWords['Add'] = 'Hinzuf�gen';
$defaultWords['Delete'] = 'L�schen';
$defaultWords['Module'] = 'Modul';
$defaultWords['Active'] = 'Aktiv';
$defaultWords['module successfully updated'] = 'Modul erfolgreich aktualisiert';
$defaultWords['module successfully removed'] = 'Modul erfolgreich entfernt';
$defaultWords['ModuleManager Mgr'] = 'ModulManager Mgr';
$defaultWords['With selected module(s)'] = 'Ausgew�hlte Module';
$defaultWords['Add a module'] = 'Modul hinzuf�gen';
$defaultWords['Module successfully added to the manager.'] = 'Modul erfolgreich hinzugef�gt';
$defaultWords['Module(s) successfully removed.'] = 'Modul(e) erfolgreich entfernt';
$defaultWords['Configurable'] = 'Konfiguierbar';
$defaultWords['Description'] = 'Beschreibung';
$defaultWords['Admin URI'] = 'Admin URI';
$defaultWords['Icon'] = 'Icon';
$defaultWords['Please, specify a name'] = 'Bitte geben Sie einen Namen an';
$defaultWords['Please, specify a title'] = 'Bitte geben Sie einen Titel an';
$defaultWords['Please, specify a description'] = 'Bitte geben Sie eine Beschreibung an';
$defaultWords['Please, specify the url to link to'] = 'Bitte geben Sie eine URL an, auf die verlinkt wird';
$defaultWords['Please, specify the name of the icon-file'] = 'Bitte geben Sie den Namen der Icon-Datei an';
$defaultWords['Username'] = 'Benutzername';
$defaultWords['Title'] = 'Titel';
$defaultWords['Status'] = 'Status';
$defaultWords['ID'] = 'ID';
$defaultWords['Name'] = 'Name';
$defaultWords['check to activate'] = 'markieren, um zu aktivieren';
$defaultWords['Enabled'] = 'Ein';
$defaultWords['Disabled'] = 'Aus';
$defaultWords['Welcome to'] = 'Willkommen bei';
$defaultWords['Home'] = 'Home';
$defaultWords['user'] = 'Benutzer';
$defaultWords['Module Manager'] = 'Modul Manager';
$defaultWords['Config Manager'] = 'Konfigurations Manager';
$defaultWords['config info successfully updated'] = 'Konfigurationsinformation erfolgreich aktualisiert';
$defaultWords['Execution Time'] = 'Execution Time';
$defaultWords['Powered by'] = 'Powered by';
$defaultWords['insufficient rights'] = 'Sie haben nicht die ben�tigten Rechte f�r diesen Bereich.';
$defaultWords['authorization required'] = 'Sie m�ssen sich einloggen, um diese Funktion zu benutzen. Bitte geben Sie Ihren Benutzernamen und Passwort an.';
$defaultWords['session timeout'] = 'Ihre Sitzung ist abgelaufen. Bitte loggen sie sich erneut ein.';
$defaultWords['You have been successfully logged out'] = 'Sie haben sich erfolgreich ausgeloggt';
$defaultWords['password emailed out'] = 'Ein neues Passwort wurde an die Emailadresse gesendet, mit der Sie sich registriert haben';
$defaultWords['email not in system'] = 'Ihr Name oder Passwort ist ung�ltig. Bitte versuchen Sie es erneut.';
$defaultWords['email submitted successfully'] = 'Ihre Email wurde erfolgreich verschickt';
$defaultWords['There was a problem sending the email'] = 'Problem: Email konnte nicht verschickt werden';
$defaultWords['message ID not recognised'] = 'Nachrichten-ID unbekannt';
$defaultWords['Please fill in the indicated fields'] = 'Bitte f�llen Sie die angezeiten Felder aus und versuchen Sie es nochmal';
$defaultWords['Your alert has been sent successfully'] = 'Ihre Fehlermeldung wurde erfolgreich verschickt';
$defaultWords['Action'] = 'Aktion';
$defaultWords['Select'] = 'Ausw�hlen';
$defaultWords['delete'] = 'l�schen';
$defaultWords['Edit'] = 'Bearbeiten';
$defaultWords['View'] = 'Ansehen';
$defaultWords['move up'] = 'nach oben';
$defaultWords['move down'] = 'nach unten';
$defaultWords['finished'] = 'Fertig';
$defaultWords['back to top'] = 'Nach oben';
$defaultWords['currently_logged_on_as'] = 'Benutzer';
$defaultWords['guest'] = 'Gast';
$defaultWords['login'] = 'Einloggen';
$defaultWords['logout'] = 'Ausloggen';
$defaultWords['session started at'] = 'Sitzung begann um';
$defaultWords['logged in at'] = 'Eingeloggt um';
$defaultWords['displaying results'] = 'Zeige Ergebnisse';
$defaultWords['to'] = 'bis';
$defaultWords['from a total of'] = 'von insgesamt';
$defaultWords['back'] = 'zur�ck';
$defaultWords['next'] = 'vor';
$defaultWords['finish'] = 'fertig';
$defaultWords['yes'] = 'ja';
$defaultWords['no'] = 'nein';
$defaultWords['Send it'] = 'Absenden';
$defaultWords['Submit'] = 'Senden';
$defaultWords['Cancel'] = 'Abbrechen';
$defaultWords['Reset'] = 'Zur�cksetzen';
$defaultWords['reset'] = 'zur�cksetzen';
$defaultWords['Save'] = 'Speichern';
$defaultWords['add'] = 'hinzuf�gen';
$defaultWords['edit'] = 'bearbeiten';
$defaultWords['move'] = 'verschieben';
$defaultWords['Manage'] = 'Verwalten';
$defaultWords['Are you sure you want to delete this'] = 'Wollen Sie das wirklich l�schen';
$defaultWords['You must select an element to delete'] = 'Sie m�ssen ein Element ausw�hlen, um es zu l�schen';
$defaultWords['no results found'] = 'Keine Ergebnisse gefunden';
$defaultWords['You have been banned'] = 'Sie wurden von dieser Seite ausgeschlossen. Bitte kontaktieren Sie den Webmaster...';
$defaultWords['Invalid POST source'] = 'Das Formular wurde von einer nicht berechtigten Quelle verschickt';
$defaultWords['You are here'] = 'Sie sind hier';
$defaultWords['whats this?'] = 'Was ist das?';
$defaultWords['denotes required field'] = 'Pflichtfeld';
$defaultWords['at time'] = 'um';
$defaultWords['Categories'] = 'Kategorien';
$defaultWords['Documents'] = 'Dokumente';
$defaultWords['Articles'] = 'Artikel';
$defaultWords['Permissions'] = 'Rechte';
$defaultWords['aMonths'][13] = 'Januar';
$defaultWords['aMonths'][14] = 'Februar';
$defaultWords['aMonths'][15] = 'M�rz';
$defaultWords['aMonths'][16] = 'April';
$defaultWords['aMonths'][17] = 'Mai';
$defaultWords['aMonths'][18] = 'Juni';
$defaultWords['aMonths'][19] = 'Juli';
$defaultWords['aMonths'][20] = 'August';
$defaultWords['aMonths'][21] = 'September';
$defaultWords['aMonths'][22] = 'Oktober';
$defaultWords['aMonths'][23] = 'November';
$defaultWords['aMonths'][24] = 'Dezember';
$defaultWords['allocated'] = 'entfallend';
$defaultWords['Editor type'] = 'Editortyp';
$defaultWords['Bug Report'] = 'Fehlerbericht';
$defaultWords['send bug report'] = 'Fehlerbericht senden';
$defaultWords['First Name'] = 'Vorname';
$defaultWords['Last Name'] = 'Nachname';
$defaultWords['You must fill in your description'] = 'Bitte geben Sie eine Beschreibung an';
$defaultWords['You must fill in your comment'] = 'Bitte geben Sie einen Kommentar ein';
$defaultWords['Your email is not correctly formatted'] = 'Ihre Emailadresse ist nicht korrekt formatiert';
$defaultWords['You must enter your email'] = 'Bitte geben Sie Ihre Emailadresse an';
$defaultWords['The name of your Seagull index file'] = 'Name der Seagull Index Datei';
$defaultWords['Currently supported editors are xinha, fck and htmlarea, and you must have the relevant libs in your www dir'] = 'Zur Zeit werden die Editoren xinha, fckEdit und htmlArea unterst�tzt. Sie m�ssen jedoch die entsprechenden Bibliotheken in Ihrem www-Verzeichnis verf�gbar haben';
$defaultWords['Show debug reporting link'] = 'Fehlerbericht-Link anzeigen';
$defaultWords['Send feedback to project for bugs'] = 'Fehlerbericht an das Seagull Projekt senden';
$defaultWords['MTA options'] = 'MTA-Optionen';
$defaultWords['Backend'] = 'Backend';
$defaultWords['PEAR::Mail backend'] = 'PEAR::Mail Backend';
$defaultWords['Sendmail path'] = 'Sendmail Pfad';
$defaultWords['Mandatory if you use Sendmail as Backend'] = 'Zwingend, wenn Sendmail als Backend verwendet wird';
$defaultWords['Sendmail arguments'] = 'Sendmail Argumente';
$defaultWords['Optional if you use Sendmail as Backend'] = 'Optional, wenn Sendmail als Backend verwendet wird';
$defaultWords['SMTP host'] = 'SMTP Host';
$defaultWords['Optional if you use SMTP as Backend. Default: localhost'] = 'Optional, wenn SMTP als Backend verwendet wird. Standard: localhost';
$defaultWords['SMTP port'] = 'SMTP Port';
$defaultWords['Optional if you use SMTP as Backend. Default: 25'] = 'Optional, wenn SMTP als Backend verwendet wird. Standard: 25';
$defaultWords['Use SMTP authentication'] = 'SMTP Anmeldung verwenden';
$defaultWords['SMTP username'] = 'SMTP Benutzername';
$defaultWords['SMTP password'] = 'SMTP Passwort';
$defaultWords['Mandatory if you use SMTP as Backend and SMTP authentication is enabled'] = 'Zwingend bei SMTP als Backend verwendet und SMTP Anmeldung';
$defaultWords['ms'] = 'ms';
$defaultWords['Extended Session'] = 'Erweiterte Session';
$defaultWords['Enforce Single User'] = 'Einzelbenutzer erzwingen';
$defaultWords['You are allowed to connect from one computer at a time, other sessions were terminated!'] = 'Sie k�nnen Sich nur von einem Computer gleichzeitig einloggen. Die anderen Sessions wurden beendet.';
$defaultWords['You have multiple sessions on this site!'] = 'Sie haben mehrere Sessions gleichzeitig bei dieser Seite';
$defaultWords['Enables extended session API when using database sessions. This allows the site to enforce one session per user.'] = 'Erlaubt erweiterte Sessions wenn datenbankbasierte Sessions verwendet werden. Dies erlaubt eine Session pro Benutzer zu erzwingen.';
$defaultWords['Enforces one session per user on this site (requires database session handling, and extended session to be on).'] = 'Erzwingt eine Session pro Benutzer. Ben�tigt Datenbankbasierte Sessions und Erweiterte Sessions.';
$defaultWords['Guests'] = 'G�ste';
$defaultWords['Members'] = 'Benutzer';
$defaultWords['Total'] = 'Gesamt';
$defaultWords['The source translation has'] = 'Das Original hat';
$defaultWords['elements'] = 'Elemente';
$defaultWords['The target translation has'] = 'Die Ziel�bersetzung hat';
$defaultWords['Please add'] = 'Bitte f�gen Sie die';
$defaultWords['values for the following keys which appear to be missing from the'] = 'Werte hinzu, die beim Modul';
$defaultWords['module'] = 'abzugehen scheinen';
$defaultWords['please specify an option'] = 'Bitte geben Sie eine Option an';
$defaultWords['please check at least one box'] = 'Bitte klicken Sie mindestens eine K�stchen an';
$defaultWords['please enter module name'] = 'Bitte geben Sie einen Modulnamen an';
$defaultWords['please enter manager name'] = 'Bitte geben Sie einen Managernamen an';
$defaultWords['navigation'] = 'Navigation';
$defaultWords['blocks'] = 'Bl�cke';
$defaultWords['categories'] = 'Kategorien';
$defaultWords['permissions'] = 'Rechte';
$defaultWords['Create a module'] = 'Modul erstellen';
$defaultWords['Manager Name'] = 'Manager Name';
$defaultWords['Create Templates'] = 'Templates erstellen';
$defaultWords['Create ini file'] = 'ini-Datei erstellen';
$defaultWords['Create Module Now'] = 'Modul jetzt erstellen';
$defaultWords['Module files successfully created'] = 'Alle Dateien des Moduls erfolgreich erstellt';
$defaultWords['Back to Maintenance'] = 'Zur�ck';
$defaultWords['File not writeable'] = 'Datei schreibgesch�tzt';
$defaultWords['Rebuild DB Sequences'] = 'DB-Sequenzen erneuern';
$defaultWords['Rebuild Sequences Now'] = 'DB-Sequenzen erneuern';
$defaultWords['validate'] = '�berpr�fen';
$defaultWords['edit'] = 'bearbeiten';
$defaultWords['Process'] = 'ausf�hren';
$defaultWords['Manage Caches'] = 'Cache verwalten';
$defaultWords['Templates'] = 'Vorlagen';
$defaultWords['Check all modules'] = 'Alle Module pr�fen';
$defaultWords['check all modules'] = 'Alle Module pr�fen';
$defaultWords['update'] = 'aktualisieren';
$defaultWords['Module Name'] = 'Modulname';
$defaultWords['ok'] = 'ok';
$defaultWords['no file'] = 'Keine Datei';
$defaultWords['new strings'] = 'Neue �bersetzungen';
$defaultWords['old strings'] = 'Alte �bersetzungen';
$defaultWords['Clear Selected Caches Now'] = 'Ausgew�hlte Cache l�schen';
$defaultWords['Rebuild Data Objects'] = 'Data Objects erneuern';
$defaultWords['Rebuild Dataobjects Now'] = 'Data Objects erneuern';
$defaultWords['Manage Translations'] = '�bersetzungen verwalten';
$defaultWords['Sequences rebuilt successfully'] = 'Sequenzen erfolgreich wiederhergestellt';
$defaultWords['Cache files successfully deleted'] = 'Cache erfolgreich gel�scht';
$defaultWords['Maintenance'] = 'Wartungsarbeiten';
$defaultWords['Congratulations, the target translation appears to be up to date'] = 'Gratulation! Ihre �bersetzung scheint auf dem letzten Stand zu sein';
$defaultWords['translation successfully updated'] = '�bersetzung erfolgreich aktualisiert';
$defaultWords['There was a problem updating the translation'] = 'Es gab ein Problem beim aktualisieren der �bersetzung';
$defaultWords['Data Objects rebuilt successfully'] = 'Data Objects erfolgreich wiederhergestellt';
$defaultWords['You are editing: Module'] = 'Sie bearbeiten: Modul';
$defaultWords['You are updating: Module'] = 'Sie aktualisieren: Modul';
$defaultWords['Master Value'] = 'Original';
$defaultWords['Translated Value'] = '�bersetzung';
$defaultWords['Save Translation'] = '�bersetzung speichern';

?>