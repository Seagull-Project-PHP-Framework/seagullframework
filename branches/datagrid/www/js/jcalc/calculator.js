...
function TCRPopup(obj_control) {
	...
	if (!this.TCRisNumber(this.control_obj.value)) alert('wrong data');
        ...
        win_ch = window.open(SGL_JS_WEBROOT + "/themes/default/default/calculator.html","win_ch", "width=" + w + ",height=" + h + ",help=no,status=no,scrollbars=no,resizable=no,top=" + n_top + ",left=" + n_left + ",dependent=yes,alwaysRaised=yes", true);
		win_ch.focus();
	}
}
