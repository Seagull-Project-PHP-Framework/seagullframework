...
include 'DataGridDataSource.php';
...
class SGL_DataGridSQLDataSource extends SGL_DataGridDataSource {
        ...
        foreach($filtersArray as $key => $filter) {
            if ($filter != '') {
                ...
                if ($pos = strpos($key, '__from__')) {
                    ...
                    $connection = current($GLOBALS['_SGL']['CONNECTIONS']);
                    if ( $connection->dbsyntax == 'pgsql') {
    ...
    function getNumberOfAllRows()
    {
        ...
        $queryCount = SGL_DataGridSQLDataSource::modifySelectQuerySetCount($this->dataGridQuery);
        $rowCount = $dbh->getOne($queryCount);
