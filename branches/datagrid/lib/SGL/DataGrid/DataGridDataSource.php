...
class SGL_DataGridDataSource {
     ...
     * @return void
     **/
    function _compareDataSource($a, $b, $element)  // KK 26941 funkcja przeniesiona jako metoda klasy
    {
        return strnatcasecmp($a["$element"], $b["$element"]);
    }
    ...
    function setSort($sortsArray)
    {
        ...
        if ($sortByElement != "") {
            usort($this->_itemsData, array("SGL_DataGridDataSource","_compareDataSource",$sortByElement));
            //set sort order
            if ($sortOrder == 'DESC') {
                //reverse data
     ...
     * @return void
     **/
    function exportPageToExcel($fileName = 'dataGridPageXLS.xls', $exportData = false)
    {
            ...
            foreach ($dataToExport as $key => $row) {
                ...
                foreach ($row as $keyRow => $data) {
                    if ($columnNotAdded) { //if columns names haven't been written yet
                        $worksheet->write($actualRow, $actualColumn, _t($keyRow), $format_bold);
                    }
                    //write actual cell in current column
                    $worksheet->write($actualRow+1 ,$actualColumn, $data);
                    $actualColumn++;
                }
