...
require_once 'DataGridColumn.php';
...
class SGL_DataGrid {
     ...
     * @return void
     **/
    function addColumn($type, $name, $dbName = array(), $sortable = false, $filterable = false,
                       $sumable = false, $sumTotalable = false, $align='')
    {
    ...
    /**
     * DataGrid::dataGridValidate()
     * Prepare and gets values from Form
     * @param object $inReq - sgl_http_request object
     * @access public
     * @return void
     **/
    function dataGridValidate(&$inReq)
    {
...
//echo $dataSource->dataGridQuery.'<br>';
        ...
        $fileName = strtr($this->dataGridHeader," ąĄćĆęĘłŁńŃśŚóÓżŻźŹ.,?:;=+<>|\/[]'",'_aAcCeElLnNsSoOzZzZ_______________');
        $fileName = strtr($fileName,'"','_');
        
        if ($fileName == '')
            $fileName = $fileName = $this->dataGridName . $this->export;
        switch($this->export) {
            case 'page_XLS':
